...
export function AppShell({
  ...
  const sidebar = <aside className="flex h-full w-64 flex-col border-r border-white/[0.07] bg-[#0c0f15]/95 p-3">
    ...
    <div className="mt-5"><DriveButton fullWidth onImported={onBookImported} /></div>
  ...
  return <div className="noise min-h-screen bg-[#090b10]"><div className="hidden h-screen md:fixed md:inset-y-0 md:left-0 md:block">{sidebar}</div>
    <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-white/[0.07] bg-[#090b10]/80 px-4 backdrop-blur md:ml-64 md:px-8"><Button variant="ghost" size="icon" className="md:hidden" onClick={() => setMobileOpen(true)}><Menu size={18} /></Button><div className="relative max-w-md flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input value={searchQuery} onChange={(event) => onSearchChange?.(event.target.value)} disabled={!onSearchChange} aria-label="Buscar na biblioteca" placeholder="Buscar por título ou autor" className="h-9 w-full rounded-md border border-white/[0.08] bg-white/[0.035] pl-9 pr-3 text-sm text-slate-200 outline-none placeholder:text-slate-600 focus:border-sky-400/60 disabled:cursor-not-allowed disabled:opacity-60" /></div><button className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-slate-300 hover:bg-white/[0.05]"><span className="grid h-6 w-6 place-items-center rounded-full bg-gradient-to-br from-sky-300 to-indigo-400 text-[10px] font-bold text-slate-950">U</span><ChevronDown size={14} className="text-slate-500" /></button></header>
