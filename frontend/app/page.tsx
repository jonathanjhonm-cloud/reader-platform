...
import { UploadBookButton, type UploadFeedback } from '@/components/upload-book-button';
...
function BookCardActions({ book, onDelete }: { book: Book; onDelete: (book: Book) => Promise<void> }) {
  ...
  return <>
    <div className="absolute right-3 top-3 z-10">
      <button type="button" aria-label={`Ações para ${book.title}`} aria-expanded={menuOpen} disabled={deleting} onClick={() => setMenuOpen((open) => !open)} className="grid h-8 w-8 place-items-center rounded-full border border-white/15 bg-[#090b10]/80 text-slate-200 shadow-lg backdrop-blur transition hover:bg-white/15 disabled:opacity-50">
        <MoreVertical size={16} />
      </button>
      {menuOpen && <div role="menu" className="absolute right-0 mt-2 w-40 rounded-lg border border-white/10 bg-[#151922] p-1.5 shadow-xl">
        <button type="button" role="menuitem" onClick={() => { setMenuOpen(false); setConfirmOpen(true); }} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs text-rose-300 transition hover:bg-rose-400/10">
          <Trash2 size={14} />Excluir livro
        </button>
      </div>}
    </div>
    <ConfirmDialog open={confirmOpen} title="Excluir livro?" description={`“${book.title}” e todo o progresso associado serão removidos da sua biblioteca.`} confirmLabel="Excluir livro" loading={deleting} onConfirm={() => void removeBook()} onClose={() => setConfirmOpen(false)} />
  </>;
}

function BookCard({ book, onDelete }: { book: Book; onDelete: (book: Book) => Promise<void> }) {
  const progress = book.progress?.percentage ?? 0;
  return <div className="relative">
    <Link href={`/books/${book.id}`} className="block">
      <Card className="group overflow-hidden transition-[transform,border-color,box-shadow] duration-300 hover:-translate-y-0.5 hover:border-white/[0.16] hover:shadow-xl hover:shadow-black/20">
        <div className="relative h-48">
          <BookCover bookId={book.id} title={book.title} available={Boolean(book.coverMimeType || book.coverUrl)} externalUrl={book.coverUrl} className="h-full" />
          <div className="absolute bottom-3 left-3 flex gap-2"><Badge>{book.fileType.toUpperCase()}</Badge>{book.contentReviewedByAi && <Badge className="bg-violet-400/15 text-violet-200">Revisado por IA</Badge>}</div>
        </div>
        <CardContent className="p-4">
          <p className="truncate text-[13px] font-medium tracking-tight text-slate-100">{book.title}</p>
          <p className="mt-1 truncate font-serif text-[11px] italic text-slate-500">{book.author || 'Autor desconhecido'}</p>
          <div className="mt-5 flex items-center justify-between text-[11px] text-slate-500"><span>{book.processingStatus === 'READY' ? `${Math.round(progress)}% lido` : book.processingStatus === 'FAILED' ? 'Falha no processamento' : 'Processando…'}</span><ArrowUpRight size={14} className="opacity-0 transition-opacity group-hover:opacity-100" /></div>
          <Progress value={progress} className="mt-2" />
        </CardContent>
      </Card>
    </Link>
    <BookCardActions book={book} onDelete={onDelete} />
  </div>;
}

function FeaturedBookSkeleton() {
  return <div className="flex min-h-48 items-stretch gap-5 p-5">
    <Skeleton className="hidden w-28 shrink-0 rounded-lg sm:block" />
    <div className="flex flex-1 flex-col justify-center">
      <Skeleton className="h-5 w-28 rounded-full" />
      <Skeleton className="mt-4 h-5 w-3/5" />
      <Skeleton className="mt-2 h-3 w-2/5" />
      <div className="mt-6 flex justify-between"><Skeleton className="h-3 w-16" /><Skeleton className="h-3 w-24" /></div>
...
function BookCardSkeleton() {
  return <Card className="overflow-hidden">
    ...
    <CardContent className="p-4">
      ...
      <Skeleton className="mt-2 h-3 w-2/5" />
      <div className="mt-5 flex justify-between"><Skeleton className="h-3 w-14" /><Skeleton className="h-3 w-4" /></div>
      <Skeleton className="mt-3 h-1.5 w-full rounded-full" />
    </CardContent>
  </Card>;
}
...
export default function Dashboard() {
  ...
  async function deleteFromLibrary(book: Book) {
    try {
      ...
      setNotice({ variant: 'success', title: 'Livro excluído', description: `“${book.title}” foi removido da sua biblioteca.` });
    } catch (cause) {
      setNotice({ variant: 'error', title: 'Falha ao excluir livro', description: cause instanceof Error ? cause.message : 'Não foi possível excluir o livro.' });
    }
  }
