...
import { ArrowLeft, CheckCircle2, ChevronLeft, ChevronRight, Loader2, MessageSquareText, Sparkles } from 'lucide-react';
...
export default function ReaderPage() {
  ...
  return <div className="min-h-screen bg-[#090b10] text-slate-200">
    <header className="sticky top-0 z-20 border-b border-white/[0.08] bg-[#090b10]/90 backdrop-blur"><div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4 sm:px-6"><Button variant="ghost" size="icon" asChild><Link href="/"><ArrowLeft size={18} /></Link></Button><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-100">{book.title}</p><div className="flex items-center gap-2 text-xs text-slate-500"><span>{Math.round(percentage)}% · {book.wordCount.toLocaleString('pt-BR')} palavras</span>{book.contentReviewedByAi && <span className="hidden items-center gap-1 text-emerald-400/80 sm:flex"><CheckCircle2 size={12} />Revisado por IA{book.removedSectionCount > 0 ? ` · ${book.removedSectionCount} removida${book.removedSectionCount > 1 ? 's' : ''}` : ''}</span>}</div></div><div className="hidden w-48 sm:block"><Progress value={percentage} /></div></div></header>
